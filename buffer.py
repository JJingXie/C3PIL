...
@dataclass
class EpisodeData:
    __slots__ = ('actor_id', 'num_agents', 'map_len', 'obs', 'urgency', 'crowd', 'last_act', 'actions', 'rewards', 'pred_d',
...
class LocalBuffer:
    __slots__ = ('actor_id', 'map_len', 'num_agents', 'obs_buf', 'urgency_buf', 'crowd_buf', 'act_buf', 'rew_buf', 'pred_d_buf', 'hidden_buf', 'forward_steps',
                'relative_pos_buf', 'q_buf', 'capacity', 'size', 'done', 'burn_in_steps', 'chunk_capacity', 'last_act_buf', 'comm_mask_buf')
    def __init__(self, actor_id: int, num_agents: int, map_len: int, init_obs: np.ndarray, init_urgency: np.ndarray, init_crowd: np.ndarray,
                 forward_steps=config.forward_steps, capacity: int = config.max_episode_length, burn_in_steps=config.burn_in_steps,
        ...
        self.urgency_buf = np.zeros((burn_in_steps+capacity+1, num_agents, 2), dtype=np.bool)
        self.crowd_buf = np.zeros((burn_in_steps+capacity+1, num_agents, 2), dtype=np.bool)
        ...
        self.pred_d_buf = np.zeros((capacity+forward_steps-1), dtype=np.float16)
        ...
        self.comm_mask_buf = np.zeros((burn_in_steps+capacity+1, 2, num_agents, num_agents), dtype=np.bool)
        ...
        self.urgency_buf[:burn_in_steps+1] = init_urgency
        self.crowd_buf[:burn_in_steps+1] = init_crowd
    
    def add(self, q_val, action: int, last_act, reward: float, pred_d: float, next_obs, next_urgency, next_crowd, hidden, relative_pos, comm_mask):
        assert self.size < self.capacity

        self.act_buf[self.size] = action
        self.rew_buf[self.size] = reward
        self.pred_d_buf[self.size] = pred_d
        self.obs_buf[self.burn_in_steps+self.size+1] = next_obs
        self.urgency_buf[self.burn_in_steps+self.size+1] = next_urgency
        self.crowd_buf[self.burn_in_steps+self.size+1] = next_crowd
    ...
    def finish(self, last_q_val=None, last_relative_pos=None, last_comm_mask=None):
        ...
        self.urgency_buf = self.urgency_buf[:self.burn_in_steps+self.size+1]
        self.crowd_buf = self.crowd_buf[:self.burn_in_steps+self.size+1]
        ...
        self.pred_d_buf = self.pred_d_buf[:self.size+self.forward_steps-1]
        ...
        data = EpisodeData(self.actor_id, self.num_agents, self.map_len, self.obs_buf, self.urgency_buf, self.crowd_buf, self.last_act_buf, self.act_buf,
                    self.rew_buf, self.pred_d_buf, self.hidden_buf, self.relative_pos_buf, self.comm_mask_buf, cumulated_gamma, td_errors, sizes, done)
