...
def main(num_actors=config.num_actors, log_interval=config.log_interval):
    ...
    expert_buffer = GlobalBuffer.remote()
    learner = Learner.remote(buffer, expert_buffer)
    time.sleep(1)
    actors = [Actor.remote(i, 0.4**(1+(i/(num_actors-1))*7), learner, buffer, expert_buffer) for i in range(num_actors)]

    for actor in actors:
        actor.run.remote()

    while not ray.get(buffer.ready.remote()) and not ray.get(expert_buffer.ready.remote()):
        time.sleep(5)
